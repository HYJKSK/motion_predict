...
class Skel:
    def __init__(self, bodytype=0,filename=os.path.join(BASEPATH,"skeleton_CMU_leg.yml")):
        if bodytype==1:
            filename = 'skeleton_CMU_leg.yml'
        elif bodytype==2:
            filename = 'skeleton_CMU_top.yml'
        else:
            filename = 'skeleton_CMU.yml'
        f = open(filename, "r")
